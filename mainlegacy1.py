...
def ceo_initiatives(priority: str = None, subtopic: str = None, topic: str = None):
    """Return initiatives filtered by topic, subtopic or priority."""
    rows = CEO_INITIATIVES
    if topic:
        rows = [r for r in rows if r.get("topic") == topic]
    if subtopic:
        rows = [r for r in rows if r.get("subtopic") == subtopic]
    if priority:
...
""",
        ...
        valid_ids = {i["id"] for i in CEO_INITIATIVES}
...
def confirm_initiative_classification(payload: dict = Body(...)):
    """Starter confirmation endpoint. Later this should save to Supabase."""
    return {
        "saved": False,
        "status": "received_confirmation_stub",
        "message": "Confirmation received. Add Supabase persistence next.",
        "payload": payload,
    }
