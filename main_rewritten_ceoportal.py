...
4. Run the Supabase SQL table setup for ceo_initiatives + initiative_email_links
"""
...
def create_initiative(payload: Dict[str, Any] = Body(...)):
    db = require_supabase()
    row = normalise_initiative(payload)
    row["id"] = row.get("id") or slugify(row.get("title", "initiative"))
    row["created_at"] = row.get("created_at") or now_iso()
    row["updated_at"] = now_iso()
    row["source"] = "user_created"

    result = db.table("ceo_initiatives").insert(row).execute()
    return {"created": True, "initiative": normalise_initiative(result.data[0]) if result.data else row}


@app.patch("/api/ceo/initiatives/{initiative_id}")
def update_initiative(initiative_id: str, payload: Dict[str, Any] = Body(...)):
    db = require_supabase()
    payload = normalise_initiative(payload)
    payload["updated_at"] = now_iso()
    payload.pop("id", None)

    result = db.table("ceo_initiatives").update(payload).eq("id", initiative_id).execute()
    return {"updated": True, "initiative": result.data[0] if result.data else None}


@app.delete("/api/ceo/initiatives/{initiative_id}")
def delete_initiative(initiative_id: str):
    db = require_supabase()
    result = db.table("ceo_initiatives").delete().eq("id", initiative_id).execute()
...
""",
    ...
    if supabase and email_id and result.get("suggested_initiative_id"):
        try:
            supabase.table("initiative_email_links").insert({
                "email_id": email_id,
                "initiative_id": result.get("suggested_initiative_id"),
                "suggested_initiative_id": result.get("suggested_initiative_id"),
...
def confirm_initiative_link(payload: Dict[str, Any] = Body(...)):
    ...
    result = db.table("initiative_email_links").insert(row).execute()
...
def dashboard():
    ...
    try:
        links = db.table("initiative_email_links").select("*").execute().data or []
        for link in links:
            if link.get("email_id"):
                initiative_by_email.setdefault(link["email_id"], []).append(link)
    except Exception:
        pass
