...
CEO_LOOKUP_TABLES = {
    # Page name / API slug -> Supabase table name.
    # Includes plural, singular, underscore and common frontend aliases.
    "stakeholders": "ceo_stakeholders",
    "stakeholder": "ceo_stakeholders",
    "organisations": "ceo_stakeholders",
    "organizations": "ceo_stakeholders",
    "contacts": "ceo_stakeholders",

    "industries": "ceo_industries",
    "industry": "ceo_industries",

    "use_cases": "ceo_use_cases",
    "usecases": "ceo_use_cases",
    "usecase": "ceo_use_cases",
    "use_case": "ceo_use_cases",

    "client_problems": "ceo_client_problems",
    "client_problem": "ceo_client_problems",
    "problems": "ceo_client_problems",
    "problem": "ceo_client_problems",

    "client_opportunities": "ceo_client_opportunities",
    "client_opportunity": "ceo_client_opportunities",
    "opportunities": "ceo_client_opportunities",
    "opportunity": "ceo_client_opportunities",

    "products": "ceo_products",
    "product": "ceo_products",

    "services": "ceo_services",
    "service": "ceo_services",

    "priorities": "ceo_priorities",
    "priority": "ceo_priorities",
    "ceo_priorities": "ceo_priorities",
    "ceo_priority": "ceo_priorities",

    "initiative_types": "ceo_initiative_types",
    "initiative_type": "ceo_initiative_types",
    "initiative-types": "ceo_initiative_types",
    "types": "ceo_initiative_types",
    "type": "ceo_initiative_types",
}

# Columns allowed for each lookup table. The frontend pages may send additional
# UI-only fields; these are ignored so Supabase inserts do not fail.
CEO_LOOKUP_COLUMNS = {
    "ceo_stakeholders": {"name", "organisation", "role", "email", "phone", "linkedin", "notes", "created_at"},
    "ceo_industries": {"name", "description", "created_at"},
    "ceo_use_cases": {"name", "description", "industry_id", "created_at"},
    "ceo_client_problems": {"name", "description", "industry_id", "use_case_id", "created_at"},
    "ceo_client_opportunities": {"name", "description", "estimated_value", "industry_id", "use_case_id", "problem_id", "created_at"},
    "ceo_products": {"name", "description", "created_at"},
    "ceo_services": {"name", "description", "created_at"},
    "ceo_priorities": {"title", "topic", "subtopic", "description", "strategic_importance", "revenue_potential", "status", "created_at"},
    "ceo_initiative_types": {"name", "description", "status", "created_at"},
}
...
INITIATIVE_COLUMNS = {
    ...
    "status", "initiative_type_id", "initiative_type", "created_at", "updated_at",
}
...
def normalise_initiative(row: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "priority": row.get("priority") or row.get("ceo_priority") or row.get("priority_id"),
        "initiative_type_id": row.get("initiative_type_id"),
        "initiative_type": row.get("initiative_type") or row.get("type"),
...
def initiative_db_payload(payload: Dict[str, Any], is_create: bool = True) -> Dict[str, Any]:
    ...
    mapped = {
        ...
        "initiative_type_id": payload.get("initiative_type_id"),
        "initiative_type": payload.get("initiative_type") or payload.get("type"),
...
def resolve_lookup_table(record_type: str) -> str:
    """Resolve a frontend page slug to a Supabase lookup table.

    This accepts many URL styles used by the frontend pages:
    priorities, priority, ceo_priorities, client-problems, usecases, etc.
    """
    key = (record_type or "").strip().lower().replace("-", "_").replace(" ", "_")
    key = key.removeprefix("ceo_") if key.startswith("ceo_ceo_") else key
    table = CEO_LOOKUP_TABLES.get(key)
    if not table:
        # Also try removing a leading ceo_ prefix.
        table = CEO_LOOKUP_TABLES.get(key.removeprefix("ceo_"))
    if not table:
        raise HTTPException(
            status_code=404,
            detail=f"Unknown record_type '{record_type}'. Use one of: {sorted(CEO_LOOKUP_TABLES.keys())}",
        )
    return table


def lookup_payload_for_table(table: str, payload: Dict[str, Any], is_create: bool = True) -> Dict[str, Any]:
    """Sanitise a lookup-table payload before saving to Supabase.

    Most UX pages use a generic form with title/name/description. Supabase
    tables differ slightly, so this maps common keys and strips anything that
    is not a real database column.
    """
    raw = dict(payload or {})
    raw.pop("id", None)

    # Convert common frontend keys into database keys.
    if table == "ceo_priorities":
        if not raw.get("title"):
            raw["title"] = raw.get("name") or raw.get("priority")
    else:
        if not raw.get("name"):
            raw["name"] = raw.get("title") or raw.get("label")

    # Common aliases from simple pages.
    if raw.get("details") and not raw.get("description"):
        raw["description"] = raw.get("details")
    if raw.get("value") and not raw.get("estimated_value"):
        raw["estimated_value"] = raw.get("value")

    allowed = CEO_LOOKUP_COLUMNS.get(table, set(raw.keys()))
    cleaned = {k: v for k, v in raw.items() if k in allowed and v not in (None, "")}

    if is_create and "created_at" in allowed:
        cleaned["created_at"] = cleaned.get("created_at") or now_iso()

    return cleaned


def list_lookup_table(record_type: str) -> List[Dict[str, Any]]:
    table = resolve_lookup_table(record_type)
    db = require_supabase()
    query = db.table(table).select("*")
    # Most tables have created_at; if one does not, Supabase will raise and we
...
def create_lookup_table_record(record_type: str, payload: Dict[str, Any]) -> Dict[str, Any]:
    table = resolve_lookup_table(record_type)
    db = require_supabase()
    cleaned = lookup_payload_for_table(table, payload, is_create=True)

    if table == "ceo_priorities" and not cleaned.get("title"):
        raise HTTPException(status_code=400, detail="title is required")
    if table != "ceo_priorities" and not cleaned.get("name"):
        raise HTTPException(status_code=400, detail="name or title is required")

    result = db.table(table).insert(cleaned).execute()
    return result.data[0] if result.data else cleaned


def update_lookup_table_record(record_type: str, record_id: str, payload: Dict[str, Any]) -> Dict[str, Any]:
    table = resolve_lookup_table(record_type)
    db = require_supabase()
    cleaned = lookup_payload_for_table(table, payload, is_create=False)
    cleaned.pop("created_at", None)
    if not cleaned:
        raise HTTPException(status_code=400, detail="No valid fields supplied")
    result = db.table(table).update(cleaned).eq("id", record_id).execute()
    return result.data[0] if result.data else cleaned


def delete_lookup_table_record(record_type: str, record_id: str) -> List[Dict[str, Any]]:
    table = resolve_lookup_table(record_type)
    db = require_supabase()
    return db.table(table).delete().eq("id", record_id).execute().data or []
...
def get_ceo_themes():
    """Return the 4 CEO themes with nested subthemes from Supabase."""
    db = require_supabase()
    themes = db.table("ceo_themes").select("*").order("sort_order").execute().data or []
    subthemes = db.table("ceo_subthemes").select("*").order("sort_order").execute().data or []

    result = []
    for theme in themes:
        item = dict(theme)
        item["subThemes"] = [s for s in subthemes if s.get("theme_id") == theme.get("id")]
        result.append(item)
    return result
...
def list_lookup_records(record_type: str):
    return list_lookup_table(record_type)


@app.post("/api/ceo/lookups/{record_type}")
def create_lookup_record(record_type: str, payload: Dict[str, Any] = Body(...)):
    return {"created": True, "record": create_lookup_table_record(record_type, payload)}
...
def update_lookup_record(record_type: str, record_id: str, payload: Dict[str, Any] = Body(...)):
    return {"updated": True, "record": update_lookup_table_record(record_type, record_id, payload)}


@app.delete("/api/ceo/lookups/{record_type}/{record_id}")
def delete_lookup_record(record_type: str, record_id: str):
    return {"deleted": True, "rows": delete_lookup_table_record(record_type, record_id)}


# Direct routes for the UX pages, e.g.:
# GET/POST /api/ceo/stakeholders, /api/ceo/products, /api/ceo/priorities
# These are aliases of the /api/ceo/lookups/{record_type} routes, but easier
# for frontend pages to call. Keep this below the specific initiatives/model
# routes so those are not shadowed.
@app.get("/api/ceo/{record_type}")
def list_ceo_records(record_type: str):
    return list_lookup_table(record_type)


@app.post("/api/ceo/{record_type}")
def create_ceo_record(record_type: str, payload: Dict[str, Any] = Body(...)):
    return {"created": True, "record": create_lookup_table_record(record_type, payload)}


@app.patch("/api/ceo/{record_type}/{record_id}")
def update_ceo_record(record_type: str, record_id: str, payload: Dict[str, Any] = Body(...)):
    return {"updated": True, "record": update_lookup_table_record(record_type, record_id, payload)}


@app.delete("/api/ceo/{record_type}/{record_id}")
def delete_ceo_record(record_type: str, record_id: str):
    return {"deleted": True, "rows": delete_lookup_table_record(record_type, record_id)}



# Extra compatibility routes for older/generated frontend pages.
# These mean the pages can call /api/ceo/records/products, /api/records/products,
# /api/products, or /api/ceo/products and still hit the same Supabase table.

@app.get("/api/ceo/records/{record_type}")
def list_ceo_records_alias(record_type: str):
    return list_lookup_table(record_type)


@app.post("/api/ceo/records/{record_type}")
def create_ceo_records_alias(record_type: str, payload: Dict[str, Any] = Body(...)):
    return {"created": True, "record": create_lookup_table_record(record_type, payload)}


@app.patch("/api/ceo/records/{record_type}/{record_id}")
def update_ceo_records_alias(record_type: str, record_id: str, payload: Dict[str, Any] = Body(...)):
    return {"updated": True, "record": update_lookup_table_record(record_type, record_id, payload)}


@app.delete("/api/ceo/records/{record_type}/{record_id}")
def delete_ceo_records_alias(record_type: str, record_id: str):
    return {"deleted": True, "rows": delete_lookup_table_record(record_type, record_id)}


@app.get("/api/records/{record_type}")
def list_records_alias(record_type: str):
    return list_lookup_table(record_type)


@app.post("/api/records/{record_type}")
def create_records_alias(record_type: str, payload: Dict[str, Any] = Body(...)):
    return {"created": True, "record": create_lookup_table_record(record_type, payload)}


@app.get("/api/{record_type}")
def list_root_record_alias(record_type: str):
    # Do not intercept known non-lookup roots.
    if record_type in {"health", "dashboard", "test-db", "ingest-demo", "ingest-email", "docs", "openapi.json"}:
        raise HTTPException(status_code=404, detail="Not a lookup record route")
    return list_lookup_table(record_type)


@app.post("/api/{record_type}")
def create_root_record_alias(record_type: str, payload: Dict[str, Any] = Body(...)):
    if record_type in {"health", "dashboard", "test-db", "ingest-demo", "ingest-email", "docs", "openapi.json"}:
        raise HTTPException(status_code=404, detail="Not a lookup record route")
    return {"created": True, "record": create_lookup_table_record(record_type, payload)}


@app.get("/api/debug/ceo-tables")
def debug_ceo_tables():
    """Check which Supabase tables are reachable from the API."""
    db = require_supabase()
    output = {
        "supabase_configured": bool(supabase),
        "initiative_table": INITIATIVES_TABLE,
        "initiative_links_table": INITIATIVE_LINKS_TABLE,
        "lookup_routes": CEO_LOOKUP_TABLES,
        "tables": {},
    }
    all_tables = sorted(set(CEO_LOOKUP_TABLES.values()) | {INITIATIVES_TABLE, INITIATIVE_LINKS_TABLE})
    for table in all_tables:
        try:
            rows = db.table(table).select("*").limit(1).execute().data
            output["tables"][table] = {"ok": True, "sample_count": len(rows or [])}
        except Exception as exc:
            output["tables"][table] = {"ok": False, "error": str(exc)}
    return output
